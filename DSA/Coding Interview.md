...
1. Change a constraint such as the data type or amount of data. This helps us simplify the problem. 
2. Solve the new simplified version of the problem. 
3. Once we have an algorithm for the simplified problem, we generalize the problem and try to adapt the earlier solution for the more complex version
...
2. This usually means recording the correct result. 
3. Try to solve the problem for n = 2, assuming that you have the answer for n = 1. 
4. Try to solve it for n = 3, assuming that you have the answer for n = 1 and n = 2
5. Build a solution that can always compute the result for N if we know the correct result for N-1 
6. It may not be until N equals 3 or 4 that we get an instance that's interesting enough to try to build the solution based on the previous result
...
This approach is useful because solving a problem may be trivial once it occurs to us to use, say, a tree. The more problems you do, the more developed your instinct on which data structure to apply will be.
#### Example
...
1. Correct: operate correctly on all expected and unexpected inputs.
2. Efficient: operate as efficiently as possible in terms of both time and space. This includes both asymptotic (big-O) efficiency and real-life efficiency. A constant factor might get dropped when you compute the big-O, but in real life, it can matter. 
3. Simple: If you can do something in 10 lines instead of 100, you should
4. Readable: A developer should be able to read your code and understand what it does and how it does it. It implements things in an easily understandable way.
5. Maintainable: Adaptable to changes during the life cycle of a product and easy to maintain by others.

It's often advisable to sacrifice some degree of efficiency to make code more maintainable, and vice versa.
...
4. Flexible and Robust
	2. Code should be flexible to different constraints. For example, don't assume board size in a board problem like tic-tac-toe
	3. Use of generics or templates when appropriate
	4. Using constants instead of variables or vice versa when appropriate
	5. Only do these things when appropriate. If the solution is complex or time won't fit then this isn't the place for it
5. Error Checking
	1. Validate the input
		1. If statements
		2. Assert statements
	2. You can skip validations in an interview by pointing out you woul
...
1. Feel free to skip initialization code and other obvious boilerplate when whiteboarding and maybe replace it with an assumed function call that does the job for you
2. Use todos in place of things that aren't core to the algorithm like error checking, validation, and testing and come back to them later if the interviewer likes
